...
from papers_agent.core.ports import GetResult
...
async def test_extract_returns_concatenated_text_in_order(
    ...
    result = await tool.run(ExtractSectionInput(paper_id="attention", section="method"))
...
async def test_extract_returns_not_found_when_empty(mock_vs: AsyncMock) -> None:
    ...
    tool = ExtractSectionTool(mock_vs)
    result = await tool.run(ExtractSectionInput(paper_id="bert", section="conclusion"))
...
async def test_extract_returns_failure_on_vs_error(mock_vs: AsyncMock) -> None:
    ...
    tool = ExtractSectionTool(mock_vs)
    result = await tool.run(ExtractSectionInput(paper_id="rag", section="abstract"))
